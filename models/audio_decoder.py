...
class AudioDecoder(nn.Module):
    def __init__(self, z_dim=64, cond_dim=128, n_mels=N_MELS, time_steps=96):
        super().__init__()
        self.h0, self.w0 = N_MELS // 16, time_steps // 16

        self.fc = nn.Linear(z_dim + cond_dim, 256 * self.h0 * self.w0)
    ...
    def forward(self, zc):
        x = self.fc(zc).view(zc.size(0), 256, self.h0, self.w0)
